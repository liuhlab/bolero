...
class mCTrainerMixin(GenericTrainer):
    ...
    def _model_validation_step(
        ...
        for batch_id, batch in enumerate(dataloader):
            y_mc, y_cov, pred_frac = self._model_forward_pass(model, batch)
            y_frac = y_mc / (y_cov + 1e-6)
            # mask is element wise mask based on coverage > cutoff
            loss_ = F.binary_cross_entropy_with_logits(pred_frac, y_frac)
            val_loss += loss_.item()
    ...
    def _plot_example_images(self, pred, label):
        epoch = self.cur_epoch + 1
        wandb_images = []

        fig, ax = plt.subplots(dpi=300)
        ds = ds = 150 / np.sqrt(label.shape[0])
        ax.scatter(label, pred, s=ds, edgecolors="none")
        ax.set_xlabel("True Fraction")
        ax.set_ylabel("Predicted Fraction")
        fig_array = figure_to_array(fig)
        fig.savefig(f"{self.savename}.example_{epoch}.jpg")
        plt.close(fig)
    ...
    def _log_save_and_check_stop(self):
        ...
        total_pearson = self.total_pearson
        ...
        print(f"Total Pearson Corr.: {total_pearson:.3f}")
            ...
            wandb.log(
                {
                    ...
                    "val/single_batch_pearson": total_pearson,
    ...
    def _fit(self, max_epochs=None, valid_first=False):
        ...
        if valid_first:
            ...
            print(f"Validation total pearson: {self.total_pearson:.3f}")
            wandb.log(
                {
                    "val/val_loss": self.val_loss,
                    "val/total_pearson": self.total_pearson,
