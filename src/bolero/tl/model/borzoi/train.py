...
from bolero.tl.generic.train_helper import CumulativeCounter, safe_save
...
class BorzoiTrainerMixin(TrainerBorzoiDatasetMixin, GenericTrainer):
    ...
    def _preemptible_save(self, cur_batches, save_path):
        from bolero.tl.generic.train_helper import safe_save
        ...
        print(f"Batch {cur_batches} checkpoint saved at {save_path}")
        return

    def _preemptible_load(self, save_path):
        self._cleanup_env()

        if not pathlib.Path(save_path).exists():
            print(f"Checkpoint {save_path} does not exist, skip loading.")
            return 0

        checkpoint: dict = torch.load(save_path, weights_only=False)
        cur_batch = checkpoint["cur_batches"]
        ...
        del checkpoint
        self._cleanup_env()
        print(f"Batch {cur_batch} checkpoint loaded from {save_path}")
        return cur_batch

    def _fit_through(self, save_path, n=500000, save_every_n=5000):
        total_batches = n // self.dataset.batch_size
        save_every_batch = save_every_n // self.dataset.batch_size
        cur_batch = self._preemptible_load(save_path)
        ...
        while cur_batch < total_batches:
            ...
            print_steps = max(5, save_every_batch // 20)
            since_last_save = 0
            for batch_id, batch in enumerate(dataloader):
                since_last_save += 1
                with self._autocast_context():
                    ...
                    if np.isnan(loss.item()):
                        raise ValueError("Training loss has NaN, stop.")
                ...
                with torch.no_grad():
                    if (batch_id + 1) % print_steps == 0:
                        ...
                        desc_str = (
                            f" - (Training) batch {cur_batch+since_last_save} "
                            f"Ave Loss: {_loss:.3f}; "
                            f"Ave Corr: {_corr} "
                            f"Last grad norm: {total_norm:.4f} "
                            f"Last batch Loss: {loss.item():.3f} "
        ...
        self.trained_n = cur_batch * self.dataset.batch_size
        return

    def _fit_through_test(self):
        self.test_loss, self.test_loss_breakdown, self.test_corr, wandb_images = (
            self._validation_step(testing=True)
        )
        wandb.log({"final_test_img": wandb_images})
        wandb.summary["trained_n"] = self.trained_n
...
class BorzoiLoRATrainer(BorzoiTrainerMixin):
    ...
    def train_through(self, save_path=None, n=600000, save_every_n=5000):
        """Train through with preemptible checkpoints, only do test in the end."""
        if save_path is None:
            save_path = self.best_checkpoint_path

        flag = pathlib.Path(f"{save_path}.success")
        ...
        with wandb_run:
            self._setup_model()
            self._setup_fit()
            self._fit_through(save_path, n=n, save_every_n=save_every_n)
            self._fit_through_test()
            self._cleanup_env()
            flag.touch()

        # cleanup checkpoint, only keep the final checkpoint
        final_ckpt = torch.load(save_path, weights_only=False)
        final_ckpt_to_keep = {k: final_ckpt[k] for k in ["cur_batches", "state_dict"]}
        safe_save(final_ckpt_to_keep, save_path)
        return
