...
class BaseAutoencoder(nn.Module):
    """Base class for sparse autoencoder models."""
    ...
    def _postprocess_output(self, x_reconstruct, x_mean, x_std):
        if self.cfg.get("input_unit_norm", False):
    ...
    def _update_inactive_features(self, acts):
...
class BatchTopKSAE(BaseAutoencoder):
    ...
    def get_loss_dict(self, x, x_reconstruct, acts, acts_topk, x_mean, x_std):
        ...
        aux_loss = self._get_auxiliary_loss(x, x_reconstruct, acts)
        loss = l2_loss + l1_loss + aux_loss
        num_dead_features = (
            self.num_batches_not_active > self.cfg["n_batches_to_dead"]
        ).sum()
        ave_batches_not_active = self.num_batches_not_active.mean()
        sae_out = self._postprocess_output(x_reconstruct, x_mean, x_std)
        output = {
            "sae_out": sae_out,
            "feature_acts": acts_topk,
            "num_dead_features": num_dead_features,
            "ave_batches_not_active": ave_batches_not_active,
            "loss": loss,
            "l1_loss": l1_loss,
            "l2_loss": l2_loss,
            "l0_norm": l0_norm,
    ...
    def _get_auxiliary_loss(self, x, x_reconstruct, acts):
