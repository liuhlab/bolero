...
class Encoder(nn.Module):
    def __init__(self, in_channel=4, output_size=256, filter_size=5, num_blocks=12):
        super().__init__()
        ...
        """
        The input and output dimensions for hidden layers for each block
        """
...
class EncoderSplit(Encoder):
    def __init__(
        ...
        self.res_blocks_seq = self.get_res_blocks(
            num_blocks, hidden_ins_half, hiddens_half
        )
        self.res_blocks_epi = self.get_res_blocks(
            num_blocks, hidden_ins_half, hiddens_half
        )
        self.conv_end = nn.Conv1d(256, output_size, 1)
...
class ResBlockDilated(nn.Module):
    def __init__(self, size, hidden=64, stride=1, dil=2):
        """
        Initialize dialted residual blocks
        """
        super().__init__()
        pad_len = dil
        self.res = nn.Sequential(
