...
class DataManager:
    ...
    def _get_condition_data(
        ...
        # pertrub_covar_df.shape[0] is the total unique combinations of perturbation covariates
        ...
        # list of list, len(perturb_covar_to_cells) == len(total_covar_combinations)
        ...
        perturbation_covariates_mask = (
            torch.from_numpy(perturbation_covariates_mask)
    ...
    def _get_split_combination_mask(
        ...
        split_combination: np.ndarray,
        split_idx_to_covariates: dict[int, tuple[Any]],
        control_mask: np.ndarray,
        src_counter: int,
    ) -> tuple[np.ndarray, dict[int, tuple[Any]], np.ndarray]:
        ...
        mask = (control_mask * split_cov_mask).astype(bool)
    ...
    def _get_split_covariates_mask(
        ...
        adata: anndata.AnnData,
        split_cov_combs: np.ndarray | list[list[Any]],  # type: ignore[type-arg]
    ) -> tuple[np.ndarray, dict[int, tuple[Any]]]:
    ...
    def _get_max_combination_length(
        ...
        if max_combination_length is None:
            ...
            print(
                f"Provided `max_combination_length` is smaller than the observed maximum combination "
                f"length of the perturbation covariates. "
                f"Setting maximum combination length to {obs_max_combination_length}.",
            )
            return obs_max_combination_length
        else:
    ...
    def _get_perturbation_covariates(
        ...
        rep_dict: dict[str, dict[str, np.ndarray]],
        ...
        for primary_cov in primary_covars:
            ...
            if primary_group in self._covariate_reps:
                ...
                prim_arr = self.primary_one_hot_encoder.transform(  # type: ignore[union-attr]
