...
from .cf_data._datamanager import DataManager
from .cf_train import BaseCallback, CellFlowTrainer
...
class CellFlow:
    ...
    def prepare_data(
        ...
        -------
            ...
            .. code-block:: python
                ...
                perturbation_covariates = {
                    {"drug": ("drug_1", "drug_2"), "dose": ("dose_1", "dose_2")}
                }
    ...
    def prepare_model(
        ...
        vf_act_type: str = "silu",
        matcher_class: dict[Literal["otcfm", "cfm", "sbcfm"], float] | None = "otcfm",
        matcher_kwargs: dict[str, Any] | None = None,
        ode_solver_kwargs: dict[str, Any] | None = None,
        ...
        if condition_mode == "stochastic":
            if not encode_conditions:
                raise ValueError(
                    "Stochastic condition embeddings require encoding conditions."
                )
        ...
        matcher_kwargs = matcher_kwargs or {"sigma": 0.0}
    ...
    def get_condition_embedding(
        ...
        for i in range(n_conditions):
            ...
            condition_embeddings_mean[c_key], condition_embeddings_var[c_key] = (
                self.solver.get_condition_embedding(condition)
            )

        df_mean = pd.DataFrame.from_dict(
            {k: v[0] for k, v in condition_embeddings_mean.items()}
        ).T
        df_var = pd.DataFrame.from_dict(
            {k: v[0] for k, v in condition_embeddings_var.items()}
        ).T
    ...
    @property
    def solver(self) -> OTFlowMatching | None:
        """The solver."""
        return self._solver
