...
class BorzoiPredictor(GenericPredictor):
            ...
            for batch in dataloader:
                ...
                if qtl_mutations:
                    ...
                    for mutation_col in mutation_cols:
                        ...
                        mutated_batch = deepcopy(batch)
                        mutated_batch["__dna__"] = mutated_batch[
                            f"__dna__:{mutation_col}"
                        ]
                        mutated_batch["allele"] = (
                            mutation_col  # Mark which allele this is
                        )
    ...
    def _check_finished_attribution_batches(
        self, batch_dir, regions_per_pseudobulk, qtl_mode=False
    ):
        """
        ...
        for path in batch_paths:
            ...
            if qtl_mode:
                ...
                companion_path = (
                    batch_dir / f"batch_{batch_idx}.{pid}.{companion_allele}.joblib.gz"
                )
    ...
    def attribution_task(
        ...
        if len(regions_per_pseudobulk_torun) != 0:
            ...
            for batch in dataloader:
                ...
                if (
                    idx == batch_counters.get(pid, 0)
                    and verbose
                    and (not first_batch_printed)
                ):
                    self._print_batch(batch, prefix="Dataloader")
        ...
        if verbose and use_qtl_mutations:
            ...
            print(
                "File naming: batch_<idx>.<pid>.ref.joblib.gz and batch_<idx>.<pid>.alt.joblib.gz"
            )
...
class BorzoiSignalPredictor(BorzoiPairPredictor):
    ...
    def _get_pre_prediction_gene_count_callbacks(self, mode):
        if mode == "eqtl":
            ...
            if hasattr(self, "qtl_manager") and self.qtl_manager is not None:
                ...
                region_name_to_strand = (
                    self.borzoi_gene_regions.borzoi_regions.set_index("Name")[
                        "Strand"
                    ].to_dict()
                )
        else:
    ...
    def _dna_attr_step(self, model, batch, attr_batch_size):
        ...
        radius = (dna.shape[-1] - dna_attr_col.shape[-1]) // 2
        batch["__dna__:attr_seq"] = dna[..., radius:-radius].detach().cpu().numpy()
        return batch
