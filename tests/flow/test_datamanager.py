...
from bolero.tl.model.flow.cf_data._datamanager import DataManager
...
class TestDataManager:
    ...
    def test_get_train_data(
        ...
        from bolero.tl.model.flow.cf_data._data import TrainingData
        from bolero.tl.model.flow.cf_data._datamanager import DataManager
        ...
        assert isinstance(list(train_data.condition_data.values())[0], torch.Tensor)
        ...
        assert isinstance(train_data.cell_data, torch.Tensor)
        assert isinstance(train_data.split_covariates_mask, torch.Tensor)
        assert isinstance(train_data.split_idx_to_covariates, dict)
        assert isinstance(train_data.perturbation_covariates_mask, torch.Tensor)
...
class TestValidationData:
    ...
    def test_get_validation_data(
        ...
        assert isinstance(val_data.cell_data, torch.Tensor)
        assert isinstance(val_data.split_covariates_mask, torch.Tensor)
        assert isinstance(val_data.split_idx_to_covariates, dict)
        assert isinstance(val_data.perturbation_covariates_mask, torch.Tensor)
        ...
        assert isinstance(list(val_data.condition_data.values())[0], torch.Tensor)
...
class TestPredictionData:
    ...
    def test_get_prediction_data(
        ...
        assert isinstance(pred_data.cell_data, torch.Tensor)
        assert isinstance(pred_data.split_covariates_mask, torch.Tensor)
        ...
        assert isinstance(list(pred_data.condition_data.values())[0], torch.Tensor)
